...
Expections: none
Notes: assumes memory accss/availability
*/
OpCodeType *addNode(OpCodeType *localPtr, OpCodeType *newNode)
{
  ...
  // return current local pointer
  return localPtr; // temporary stub return
}
...
Algorithm: iterates though op code linked list,
           displays op code data invidually
Precondition: linked list, with or without data
...
int getCommand(char *cmd, char *inputStr, int index)
{
   ...
   {
     // assign character form input stirng to buffer string
   ...
   return index; // temporary stub return
}
...
Postcondition: returns succces (Boolean), returns linked list head via parameter,
...
Precondition: file is open and file cursor is at beginnign
              of an op code
...
int getOpCommand(FILE *filePtr, OpCodeType *inData)
{
  ...
  else
  {
    ...
    // return op command access faiure
...
Algorithm: skips leaing white space,
...
Postcondition: in correct operation,
               ...
               returns index location afer process finished
Exceptions: none
Notes: none
*/
...
Algorithm: skips learing while space,
...
int getStringArg(char *strArg, char *inputStr, int index)
{
  ...
  {
    // assign charaacter from input string to buffer string
