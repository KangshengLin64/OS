...
Boolean runProcess(process_t *current_process, ConfigDataType *configDataType){
  ...
  for(i=0;i< current_process->exe_size;++i){
   ...
   case DEVOUT:{
    ...
    char*value =(char*) malloc(sizeof(char)*1000);
    sprintf(value,"Process: %d device output %s start, time: %d ms.", running_process_identifier, current_process->execution_flow[i].origin->strArg1, current_process->execution_flow[i].intArg2 * configDataType-> quantumCycles * configDataType-> ioCycleRate);
    output(value,configDataType);
    timer(current_process->execution_flow[i].intArg2 * configDataType->quantumCycles * configDataType->ioCycleRate);
   ...
   case MEM:{
    ...
    if(current_process->execution_flow[i].strArgs1==ALLOCATE){
     ...
     else sprintf(value,"Process: %d Memory allocation from %d to %d failed.", running_process_identifier,  current_process->execution_flow[i].intArg2, current_process->execution_flow[i].intArg3);
     ...
     char *value= (char*) malloc(sizeof(char)*1000);
     sprintf(value,"Process: %d Memory access to %d.", running_process_identifier, current_process->execution_flow[i].intArg2);
     output(value, configDataType);
     Boolean result = mem_access_func(current_process->execution_flow[i].intArg2);

      if(result) sprintf(value,"Process: %d Memory access to %d success.", running_process_identifier, current_process->execution_flow[i].intArg2);

      else sprintf(value,"Process: %d Memory access to %d failed.", running_process_identifier,current_process->execution_flow[i].intArg2);
